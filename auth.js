...
async function handleEmailSignIn(email, password) {
    try {
        ...
        showAuthError(getAuthErrorMessage(e.code));
        throw e; // Re-throw to handle in form submit
    }
}
 ...
 * Made global for inline onclick handler
 */
...
async function onAppPageReady(firebaseUser) {
    try {
        ...
        // Update UI elements (check if they exist)
        ...
        if (typeof dom !== 'undefined') {
            ...
            // Direct DOM manipulation if dom object not ready
...
function setupAuthListeners() {
    ...
    if (googleBtn) {
        ...
        const newGoogleBtn = document.getElementById('btn-google-signin');
    ...
    if (authForm) {
        authForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                // Error already shown, just reset button
            }
    ...
    // Sign out button (if dom exists)
    if (typeof dom !== 'undefined' && dom.btnSignOut) {
        dom.btnSignOut.addEventListener('click', handleSignOut);
    }
    
    // Also try direct DOM query for sign out
...
// Also ensure global access for functions that might be called from other scripts
