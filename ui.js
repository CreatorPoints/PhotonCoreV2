...
function loadSavedModel(){try{const s=localStorage.getItem('photon_selected_model');if(s&&AI_MODELS[s]){state.selectedModel=s;if(dom.aiModelSelect)dom.aiModelSelect.value=s}else{state.selectedModel=DEFAULT_MODEL;if(dom.aiModelSelect)dom.aiModelSelect.value=DEFAULT_MODEL}}catch(e){state.selectedModel=DEFAULT_MODEL;if(dom.aiModelSelect)dom.aiModelSelect.value=DEFAULT_MODEL}updateBotIdentity()}

function loadTipState(){try{const v=localStorage.getItem('photon_tip_dismissed');if(v==='true'&&dom.memoryTipBanner)dom.memoryTipBanner.classList.add('dismissed')}catch(e){}}
