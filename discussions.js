...
function listenDiscussions(){
    db.collection('discussions').orderBy('timestamp','desc').limit(100).onSnapshot(snap=>{state.discussions=[];snap.forEach(doc=>state.discussions.push({id:doc.id,...doc.data()}));renderDiscussions();if(dom.statDiscussions)dom.statDiscussions.textContent=state.discussions.length});
}
...
async function postDiscussion(){
    ...
    await db.collection('discussions').add({title:t,body:b,category:dom.discussionCategory.value,author:state.user?.username||'Anon',timestamp:new Date().toISOString(),likes:0});
    addActivity('💬 '+state.user?.username+': "'+t+'"');dom.discussionTitle.value='';dom.discussionBody.value='';showToast('Posted!','success');
}

function renderDiscussions(){
    if(!dom.discussionsList)return;
    const f=state.currentFilter==='all'?state.discussions:state.discussions.filter(d=>d.category===state.currentFilter);
    if(!f.length){dom.discussionsList.innerHTML='<div class="empty-state"><span class="empty-icon">💬</span><p>No discussions.</p></div>';return}
